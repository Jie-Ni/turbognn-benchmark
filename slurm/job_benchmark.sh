...
# Full-node job: runs up to 4 tasks in parallel (1 per GPU)
# Usage: sbatch --array=0-N job_benchmark.sh <TASK_FILE>
# Each array element processes 4 consecutive tasks from TASK_FILE
...
TASK_FILE="${1:?Usage: sbatch --array=0-N job_benchmark.sh <task_file>}"
...
# Each array element handles tasks: ARRAY_ID*4 through ARRAY_ID*4+3
PIDS=()
for GPU_IDX in 0 1 2 3; do
    ...
    GRAPH_INSTANCE_SEED=$(echo "$LINE" | awk '{print $5}')
    GRAPH_INSTANCE_ARGS=()
    ...
    CUDA_VISIBLE_DEVICES=$GPU_IDX python run_benchmark.py \
        ...
        > "$WORK_DIR/logs/task_${TASK_IDX}.log" 2>&1 &
    PIDS+=($!)
done
