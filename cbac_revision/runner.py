...
def preflight_matched_benchmark(settings: RunnerSettings) -> PreflightBundle:
    ...
    if settings.data_path.suffix.casefold() == ".h5ad":
        ...
        dataset_passport = load_dataset_passport(
            ...
            expected_control_label_counts=(
                dataset_protocol["control"].get("evidence", {}).get("exact_archived_label_counts")
                if settings.dataset_name == "adamson"
                else None
            ),
            expected_control_cell_count=(
                dataset_protocol["control"].get("evidence", {}).get("expected_control_cell_count")
                if settings.dataset_name == "adamson"
                else None
            ),
        )
    ...
    if dataset_passport is not None:
        ...
        if settings.dataset_name == "adamson" and (
            observed_control_counts != {"62(mod)_pBA581": 2, "63(mod)_pBA580": 6010}
            or sum(observed_control_counts.values()) != 6012
        ):
            raise PreflightBlockedError(
                "[ADAMSON_CONTROL_ROW_COUNT_MISMATCH] expected=2+6010=6012;"
                f"observed={observed_control_counts}"
            )
    ...
    matched_control_label_counts = {
        label: int(np.sum(np.asarray(dataset.condition_labels, dtype=object) == label))
        for label in resolution.matched_raw_labels
    }
    ...
    condition_panel_manifest = _build_condition_panel_manifest(
        ...
        matched_control_label_counts=matched_control_label_counts,
        panels=panels,
...
def execute_matched_benchmark(bundle: PreflightBundle) -> list[Path]:
    ...
    for held_out in bundle.selected_conditions:
        ...
        if baseline_path.exists():
            ...
            if (
                baseline_payload.get("gene_names") != list(bundle.preprocessor.state.selected_genes)
                or baseline_payload.get("training_condition_ids") != train_conditions
                or baseline_payload.get("held_out_canonical_targets")
                != list(encoding.canonical_targets)
                or baseline_payload.get("y_true")
                != (bundle.condition_profiles[held_out] - bundle.control_profile).tolist()
                or baseline_payload.get("input_hashes") != bundle.input_hashes
                or baseline_payload.get("preprocessing_state_hash")
                != bundle.preprocessor.state.sha256()
            ):
                raise RevisionProtocolError(
                    "[BASELINE_ARTIFACT_UPSTREAM_BINDING_MISMATCH] Existing artifact cannot "
                    "be reused against different truth/gene/target/preprocessing inputs"
                )
        else:
            ...
            baseline_payload = build_baseline_artifact(
                ...
                held_out_canonical_targets=encoding.canonical_targets,
...
def _build_condition_panel_manifest(
    *,
    ...
    matched_control_label_counts: Mapping[str, int],
    panels: ConditionPanels,
    ...
    payload: dict[str, Any] = {
        ...
        "control_binding": {
            ...
            "matched_raw_label_counts": {
                label: int(matched_control_label_counts[label]) for label in matched_control_labels
            },
            "matched_control_cell_count": int(sum(matched_control_label_counts.values())),
            "count_binding_status": (
                "PASS_HASH_VERIFIED_DATASET_ROWS"
                if dataset_passport_hash is not None
                else "PASS_SYNTHETIC_FIXTURE_ROWS"
            ),
            "binding_source": (
