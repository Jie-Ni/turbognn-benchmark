...
def load_dataset_passport(
    ...
    *,
    ...
    expected_control_label_counts: Mapping[str, int] | None = None,
    expected_control_cell_count: int | None = None,
) -> DatasetPassport:
    ...
    if dataset_name == "adamson":
        expected_counts = (
            dict(expected_control_label_counts)
            if isinstance(expected_control_label_counts, Mapping)
            else None
        )
        if (
            expected_counts is None
            or set(expected_counts) != {"62(mod)_pBA581", "63(mod)_pBA580"}
            or expected_counts != {"62(mod)_pBA581": 2, "63(mod)_pBA580": 6010}
            or any(
                isinstance(value, bool) or not isinstance(value, int) or value <= 0
                for value in expected_counts.values()
            )
            or isinstance(expected_control_cell_count, bool)
            or not isinstance(expected_control_cell_count, int)
            or expected_control_cell_count <= 0
            or expected_control_cell_count != 6012
            or sum(expected_counts.values()) != expected_control_cell_count
        ):
            raise RevisionProtocolError("[ADAMSON_CONTROL_COUNT_CONTRACT_MISSING]")
        if (
            raw_label_counts != expected_counts
            or sum(raw_label_counts.values()) != expected_control_cell_count
        ):
            raise RevisionProtocolError("[ADAMSON_CONTROL_EVIDENCE_COUNT_MISMATCH]")
